"""Passive telemetry planning and metadata ingestion helpers.

The telemetry package builds local metadata, dry-run planning records, and
bounded packet metadata windows only. It does not persist raw payloads, modify
traffic, inject traffic, automatically block anything, or start hidden monitors.
"""

from core_engine.telemetry.capture_sessions import (
    CAPTURE_SESSION_MODES,
    CAPTURE_SESSION_RECORD_VERSION,
    PassiveCaptureSessionError,
    build_capture_plan_api_response,
    build_capture_plan_dashboard_record,
    build_capture_targets,
    build_passive_capture_session_plan,
    deterministic_capture_plan_json,
    summarize_capture_plan,
    validate_capture_session_plan_inputs,
)
from core_engine.telemetry.baseline_windows import (
    BASELINE_WINDOW_RECORD_VERSION,
    BASELINE_WINDOW_SAFETY_FLAGS,
    DEFAULT_BASELINE_WINDOWS,
    BaselineWindowError,
    build_baseline_window_record,
    build_baseline_window_records,
    deterministic_baseline_window_json,
    summarize_baseline_windows,
)
from core_engine.telemetry.anomaly_windows import (
    ANOMALY_WINDOW_RECORD_VERSION,
    ANOMALY_WINDOW_SAFETY_FLAGS,
    DEFAULT_MAX_ANOMALY_WINDOW_KEYS,
    AnomalyWindowError,
    build_anomaly_window_record,
    build_anomaly_window_records,
    deterministic_anomaly_window_json,
    summarize_anomaly_windows,
)
from core_engine.telemetry.risk_weights import (
    ADAPTIVE_RISK_WEIGHT_RECORD_VERSION,
    ADAPTIVE_RISK_SAFETY_FLAGS,
    apply_adaptive_risk_weights,
    build_risk_adjustment,
    burst_anomaly_adjustment,
    clamp_risk_score,
    deterministic_risk_weight_json,
    low_confidence_dampening_adjustment,
    mature_baseline_confidence_adjustment,
    new_behavior_adjustment,
    stable_behavior_adjustment,
    unusual_destination_adjustment,
    unusual_service_fingerprint_adjustment,
)
from core_engine.telemetry.adaptive_risk import (
    ADAPTIVE_RISK_RECORD_VERSION,
    DEFAULT_ADAPTIVE_RISK_BASE_SCORE,
    AdaptiveRiskError,
    build_adaptive_risk_api_response,
    build_adaptive_risk_dashboard_record,
    build_adaptive_risk_explanation,
    build_adaptive_risk_export_record,
    build_adaptive_risk_operator_panel,
    build_adaptive_risk_record,
    build_adaptive_risk_report,
    build_anomaly_risk_context,
    build_baseline_risk_context,
    build_destination_risk_context,
    build_service_fingerprint_risk_context,
    deterministic_adaptive_risk_json,
    summarize_adaptive_risk_records,
)
from core_engine.telemetry.behavior_summary import (
    BEHAVIORAL_INTELLIGENCE_RECORD_VERSION,
    BEHAVIORAL_INTELLIGENCE_SAFETY_FLAGS,
    build_adaptive_risk_rollup,
    build_baseline_summary_rollup,
    build_behavioral_explanation_records,
    build_behavioral_intelligence_api_response,
    build_behavioral_intelligence_dashboard_record,
    build_behavioral_intelligence_export_record,
    build_behavioral_intelligence_summary,
    build_behavioral_privacy_safety_summary,
    build_behavioral_recommendation_records,
    build_behavioral_state_summary,
    build_dns_destination_behavior_rollup,
    build_service_fingerprint_rollup,
    build_temporal_anomaly_rollup,
    deterministic_behavioral_intelligence_json,
)
from core_engine.telemetry.behavior_operator_views import (
    build_behavioral_intelligence_empty_state,
    build_behavioral_intelligence_operator_view,
    build_behavioral_intelligence_status_panel,
    deterministic_behavior_operator_view_json,
)
from core_engine.telemetry.behavior_baselines import (
    BASELINE_CATEGORIES,
    BEHAVIOR_BASELINE_RECORD_VERSION,
    BEHAVIOR_BASELINE_SAFETY_FLAGS,
    DEFAULT_BASELINE_MATURITY_THRESHOLD,
    DEFAULT_MAX_BASELINE_ENTRIES,
    BehaviorBaselineError,
    build_baseline_entries,
    build_baseline_observations,
    build_behavior_baseline_api_response,
    build_behavior_baseline_dashboard_record,
    build_behavior_baseline_export_record,
    build_behavior_baseline_report,
    classify_baseline_behavior,
    deterministic_behavior_baseline_json,
    score_baseline_confidence,
    summarize_behavior_baselines,
)
from core_engine.telemetry.temporal_anomalies import (
    DEFAULT_BURST_MULTIPLIER,
    DEFAULT_MAX_TEMPORAL_ANOMALIES,
    DEFAULT_MIN_BURST_COUNT,
    DEFAULT_VOLUME_DRIFT_MULTIPLIER,
    TEMPORAL_ANOMALY_RECORD_VERSION,
    TEMPORAL_ANOMALY_SAFETY_FLAGS,
    TemporalAnomalyError,
    build_temporal_anomaly_api_response,
    build_temporal_anomaly_dashboard_record,
    build_temporal_anomaly_export_record,
    build_temporal_anomaly_report,
    detect_burst_anomalies,
    detect_rare_service_timing,
    detect_volume_drift_hints,
    detect_window_novelty,
    deterministic_temporal_anomaly_json,
    score_anomaly_confidence,
    summarize_temporal_anomalies,
)
from core_engine.telemetry.fingerprint_profiles import (
    DEFAULT_MAX_SERVICE_FINGERPRINT_PROFILES,
    DEFAULT_SERVICE_FINGERPRINT_MATURITY_THRESHOLD,
    SERVICE_FINGERPRINT_PROFILE_RECORD_VERSION,
    SERVICE_FINGERPRINT_PROFILE_SAFETY_FLAGS,
    ServiceFingerprintProfileError,
    build_fingerprint_profile_records,
    build_service_fingerprint_api_response,
    build_service_fingerprint_dashboard_record,
    build_service_fingerprint_export_record,
    classify_service_fingerprint_profile,
    deterministic_service_fingerprint_profile_json,
    score_service_fingerprint_confidence,
    summarize_service_fingerprint_profiles,
)
from core_engine.telemetry.dns_correlation import (
    build_dns_visibility_report,
    correlate_domains_to_flows,
    deterministic_dns_correlation_json,
    pair_dns_queries_and_responses,
)
from core_engine.telemetry.destination_learning import (
    DEFAULT_DESTINATION_MATURITY_THRESHOLD,
    DEFAULT_MAX_DESTINATION_BEHAVIOR_RECORDS,
    DESTINATION_LEARNING_RECORD_VERSION,
    DESTINATION_LEARNING_SAFETY_FLAGS,
    DestinationLearningError,
    build_destination_learning_records,
    classify_destination_ip_placeholder,
    deterministic_destination_learning_json,
    safe_destination_domain_summary,
    score_destination_learning_confidence,
)
from core_engine.telemetry.dns_behavior import (
    DNS_BEHAVIOR_RECORD_VERSION,
    DNS_BEHAVIOR_SAFETY_FLAGS,
    build_dns_behavior_profiles,
    build_dns_destination_api_response,
    build_dns_destination_behavior_report,
    build_dns_destination_dashboard_record,
    build_dns_destination_export_record,
    build_dns_destination_operator_panel,
    classify_dns_destination_behavior,
    deterministic_dns_destination_behavior_json,
    summarize_dns_destination_behavior,
)
from core_engine.telemetry.dns_visibility import (
    DEFAULT_DOMAIN_LABEL_LIMIT,
    DEFAULT_DOMAIN_TOTAL_LIMIT,
    DNS_ERROR_CODES,
    DNS_VISIBILITY_RECORD_VERSION,
    DNS_VISIBILITY_SAFETY_FLAGS,
    build_dns_anomaly_hints,
    build_dns_query_record,
    build_dns_response_record,
    build_dns_timing_summary,
    build_dns_visibility_api_response,
    build_dns_visibility_dashboard_record,
    build_encrypted_dns_limitation_summary,
    classify_resolver,
    deterministic_dns_visibility_json,
    normalize_dns_answer,
    safe_dns_text,
    sanitize_domain_name,
    summarize_dns_visibility_records,
)
from core_engine.telemetry.flows import (
    DEFAULT_EPHEMERAL_PORT_MIN,
    DEFAULT_PERSISTENT_BYTE_THRESHOLD,
    DEFAULT_PERSISTENT_PACKET_THRESHOLD,
    FLOW_RECORD_VERSION,
    SERVICE_PORTS,
    FlowReconstructionError,
    associate_flow_service,
    build_flow_api_response,
    build_flow_dashboard_record,
    classify_flow_persistence,
    classify_flow_record,
    deterministic_flow_json,
    flow_digest,
    flow_to_topology_edge,
    normalize_flow_key,
    reconstruct_flow_record,
    summarize_flows,
)
from core_engine.telemetry.flow_enrichment import (
    DEFAULT_MAX_ENRICHED_FLOW_OBSERVATIONS,
    FLOW_ENRICHMENT_RECORD_VERSION,
    FlowEnrichmentError,
    build_flow_enrichment_api_response,
    build_flow_enrichment_dashboard_record,
    build_rolling_flow_statistics,
    deterministic_flow_enrichment_json,
    enrich_flow_records,
    summarize_enriched_flow_observations,
)
from core_engine.telemetry.flow_observations import (
    DEFAULT_LOCAL_CIDRS,
    FLOW_OBSERVATION_RECORD_VERSION,
    FLOW_OBSERVATION_SAFETY_FLAGS,
    build_enriched_flow_observation,
    build_flow_counter_summary,
    build_flow_quality_flags,
    build_flow_state_transition,
    classify_endpoint_scope,
    classify_flow_endpoints,
    correlate_service_port_hint,
    deterministic_flow_observation_json,
    infer_flow_direction,
    score_flow_observation_confidence,
)
from core_engine.telemetry.fingerprints import (
    DEFAULT_METADATA_FIELD_LIMIT,
    PROTOCOL_DEFAULT_PORTS,
    PROTOCOL_FINGERPRINT_RECORD_VERSION,
    SENSITIVE_METADATA_KEYS,
    SERVICE_PROTOCOL_HINTS,
    build_protocol_fingerprint,
    build_service_fingerprint_summary,
    deterministic_protocol_fingerprint_json,
    infer_protocol_hint,
    safe_truncate_metadata_value,
    sanitize_metadata_fields,
    score_protocol_confidence,
)
from core_engine.telemetry.interfaces import (
    INTERFACE_RECORD_VERSION,
    TELEMETRY_SAFETY_FLAGS,
    TelemetryInterfaceError,
    build_interface_api_response,
    build_interface_dashboard_record,
    build_interface_resource_budget_summary,
    classify_interface_capabilities,
    deterministic_interface_json,
    enumerate_local_interfaces,
    normalize_address_family,
    normalize_interface_address,
    normalize_interface_metadata,
    summarize_interface_address_families,
    summarize_interfaces,
)
from core_engine.telemetry.ingestion import (
    PACKET_METADATA_RECORD_VERSION,
    PAYLOAD_FIELD_NAMES,
    SUPPORTED_TRANSPORTS,
    PacketIngestionError,
    classify_packet_address_family,
    classify_packet_record,
    classify_transport_protocol,
    deterministic_packet_metadata_json,
    normalize_packet_metadata,
    normalize_packet_metadata_batch,
    packet_metadata_digest,
    summarize_packet_metadata_records,
    summarize_packet_sizes,
    validate_ip_address,
)
from core_engine.telemetry.live_topology import (
    build_cluster_federation_topology_summary,
    build_live_topology,
    build_live_topology_api_response,
    build_live_topology_dashboard_record,
    deterministic_live_topology_record_json,
)
from core_engine.telemetry.packet_window import (
    DEFAULT_MAX_WINDOW_BYTES,
    DEFAULT_MAX_WINDOW_PACKETS,
    EDGE_MAX_WINDOW_BYTES,
    EDGE_MAX_WINDOW_PACKETS,
    PACKET_WINDOW_RECORD_VERSION,
    PacketWindowError,
    build_packet_ingestion_api_response,
    build_packet_ingestion_dashboard_record,
    build_packet_ingestion_window,
    classify_window_packet,
    deterministic_packet_window_json,
    summarize_packet_ingestion_window,
)
from core_engine.telemetry.process_attribution import (
    PROCESS_ATTRIBUTION_RECORD_VERSION,
    PROCESS_ATTRIBUTION_SAFETY_FLAGS,
    attribute_process_to_flow,
    build_process_inventory_api_response,
    build_process_inventory_dashboard_record,
    build_process_socket_inventory,
    build_process_socket_inventory_from_platform,
    deterministic_process_attribution_json,
    minimize_process_metadata,
    normalize_socket_record,
    summarize_socket_inventory,
)
from core_engine.telemetry.operator_views import (
    DEFAULT_STALE_AFTER_SECONDS,
    DEFAULT_UPDATE_INTERVAL_SECONDS,
    LIVE_TELEMETRY_VIEW_RECORD_VERSION,
    LIVE_TELEMETRY_VIEW_SAFETY_FLAGS,
    MAX_UPDATE_INTERVAL_SECONDS,
    MIN_UPDATE_INTERVAL_SECONDS,
    build_bounded_update_interval_controls,
    build_empty_live_telemetry_model,
    build_federation_aware_telemetry_rollup,
    build_flow_rate_summary,
    build_interface_telemetry_summary,
    build_live_telemetry_api_response,
    build_live_telemetry_operator_summary,
    build_live_topology_rendering_summary,
    build_packet_rate_summary,
    build_process_service_attribution_operator_panel,
    build_adaptive_risk_operator_panel,
    build_behavior_baseline_operator_panel,
    build_dns_destination_operator_panel,
    build_service_fingerprint_operator_panel,
    build_temporal_anomaly_operator_panel,
    build_protocol_distribution_summary,
    build_resource_usage_telemetry_summary,
    build_stale_telemetry_state_model,
    build_telemetry_health_status_summary,
    deterministic_live_telemetry_json,
    summarize_live_telemetry_panels,
)
from core_engine.telemetry.service_fingerprints import (
    DEFAULT_MAX_SERVICE_BEHAVIOR_FINGERPRINTS,
    SERVICE_BEHAVIOR_FINGERPRINT_RECORD_VERSION,
    SERVICE_BEHAVIOR_FINGERPRINT_SAFETY_FLAGS,
    ServiceBehaviorFingerprintError,
    build_service_behavior_fingerprint_report,
    build_service_fingerprint_record,
    build_service_fingerprint_records,
    classify_service_fingerprint,
    deterministic_service_behavior_fingerprint_json,
    score_service_fingerprint_record,
    summarize_service_fingerprints,
)
from core_engine.telemetry.service_attribution import (
    SERVICE_ATTRIBUTION_RECORD_VERSION,
    SERVICE_ATTRIBUTION_SAFETY_FLAGS,
    build_process_service_attribution_report,
    build_sanitized_attribution_display,
    build_service_attribution_api_response,
    build_service_attribution_dashboard_record,
    build_service_attribution_record,
    deterministic_service_attribution_json,
    score_service_attribution_confidence,
    summarize_service_attributions,
)
from core_engine.telemetry.protocol_metadata import (
    DNS_ALLOWED_FIELDS,
    HTTP_ALLOWED_FIELDS,
    PROTOCOL_METADATA_RECORD_VERSION,
    TLS_ALLOWED_FIELDS,
    ProtocolMetadataError,
    build_metadata_governance_summary,
    build_protocol_anomaly_summaries,
    build_protocol_api_response,
    build_protocol_dashboard_record,
    deterministic_protocol_metadata_json,
    extract_dns_metadata,
    extract_http_metadata,
    extract_protocol_metadata,
    extract_protocol_metadata_report,
    extract_tls_metadata,
    summarize_protocol_metadata,
)
from core_engine.telemetry.session_tracker import (
    DEFAULT_FLOW_TIMEOUT_SECONDS,
    SESSION_TRACKER_RECORD_VERSION,
    FlowSessionTrackerError,
    build_flow_session_records,
    deterministic_session_tracker_json,
    reconstruct_flows_from_packet_window,
    summarize_flow_sessions,
    track_flow_sessions,
)
from core_engine.telemetry.topology_correlation import (
    DEFAULT_MAX_LIVE_EDGES,
    DEFAULT_MAX_LIVE_NODES,
    LIVE_TOPOLOGY_RECORD_VERSION,
    build_protocol_aware_topology_summary,
    build_temporal_topology_summary,
    build_topology_health_summary,
    build_topology_update_record,
    correlate_flow_topology_edges,
    correlate_topology_drift,
    deterministic_live_topology_json,
    infer_live_node_relationships,
    infer_node_roles,
)

__all__ = [
    "CAPTURE_SESSION_MODES",
    "CAPTURE_SESSION_RECORD_VERSION",
    "ANOMALY_WINDOW_RECORD_VERSION",
    "ANOMALY_WINDOW_SAFETY_FLAGS",
    "ADAPTIVE_RISK_RECORD_VERSION",
    "ADAPTIVE_RISK_SAFETY_FLAGS",
    "ADAPTIVE_RISK_WEIGHT_RECORD_VERSION",
    "BASELINE_CATEGORIES",
    "BASELINE_WINDOW_RECORD_VERSION",
    "BASELINE_WINDOW_SAFETY_FLAGS",
    "BEHAVIOR_BASELINE_RECORD_VERSION",
    "BEHAVIOR_BASELINE_SAFETY_FLAGS",
    "BEHAVIORAL_INTELLIGENCE_RECORD_VERSION",
    "BEHAVIORAL_INTELLIGENCE_SAFETY_FLAGS",
    "DESTINATION_LEARNING_RECORD_VERSION",
    "DESTINATION_LEARNING_SAFETY_FLAGS",
    "DEFAULT_BASELINE_MATURITY_THRESHOLD",
    "DEFAULT_BASELINE_WINDOWS",
    "DEFAULT_DESTINATION_MATURITY_THRESHOLD",
    "DEFAULT_BURST_MULTIPLIER",
    "DEFAULT_EPHEMERAL_PORT_MIN",
    "DEFAULT_DOMAIN_LABEL_LIMIT",
    "DEFAULT_DOMAIN_TOTAL_LIMIT",
    "DEFAULT_ADAPTIVE_RISK_BASE_SCORE",
    "DEFAULT_FLOW_TIMEOUT_SECONDS",
    "DEFAULT_LOCAL_CIDRS",
    "DEFAULT_MAX_ENRICHED_FLOW_OBSERVATIONS",
    "DEFAULT_MAX_BASELINE_ENTRIES",
    "DEFAULT_MAX_ANOMALY_WINDOW_KEYS",
    "DEFAULT_MAX_TEMPORAL_ANOMALIES",
    "DEFAULT_MAX_DESTINATION_BEHAVIOR_RECORDS",
    "DEFAULT_MAX_SERVICE_BEHAVIOR_FINGERPRINTS",
    "DEFAULT_MAX_SERVICE_FINGERPRINT_PROFILES",
    "DEFAULT_MAX_LIVE_EDGES",
    "DEFAULT_MAX_LIVE_NODES",
    "DEFAULT_MAX_WINDOW_BYTES",
    "DEFAULT_MAX_WINDOW_PACKETS",
    "DEFAULT_METADATA_FIELD_LIMIT",
    "DEFAULT_MIN_BURST_COUNT",
    "DEFAULT_PERSISTENT_BYTE_THRESHOLD",
    "DEFAULT_PERSISTENT_PACKET_THRESHOLD",
    "DEFAULT_STALE_AFTER_SECONDS",
    "DEFAULT_UPDATE_INTERVAL_SECONDS",
    "DEFAULT_VOLUME_DRIFT_MULTIPLIER",
    "DEFAULT_SERVICE_FINGERPRINT_MATURITY_THRESHOLD",
    "DNS_ERROR_CODES",
    "DNS_BEHAVIOR_RECORD_VERSION",
    "DNS_BEHAVIOR_SAFETY_FLAGS",
    "DNS_VISIBILITY_RECORD_VERSION",
    "DNS_VISIBILITY_SAFETY_FLAGS",
    "EDGE_MAX_WINDOW_BYTES",
    "EDGE_MAX_WINDOW_PACKETS",
    "FLOW_RECORD_VERSION",
    "FLOW_ENRICHMENT_RECORD_VERSION",
    "FLOW_OBSERVATION_RECORD_VERSION",
    "FLOW_OBSERVATION_SAFETY_FLAGS",
    "INTERFACE_RECORD_VERSION",
    "LIVE_TOPOLOGY_RECORD_VERSION",
    "LIVE_TELEMETRY_VIEW_RECORD_VERSION",
    "LIVE_TELEMETRY_VIEW_SAFETY_FLAGS",
    "MAX_UPDATE_INTERVAL_SECONDS",
    "MIN_UPDATE_INTERVAL_SECONDS",
    "PACKET_METADATA_RECORD_VERSION",
    "PACKET_WINDOW_RECORD_VERSION",
    "PROCESS_ATTRIBUTION_RECORD_VERSION",
    "PROCESS_ATTRIBUTION_SAFETY_FLAGS",
    "PAYLOAD_FIELD_NAMES",
    "PROTOCOL_DEFAULT_PORTS",
    "PROTOCOL_FINGERPRINT_RECORD_VERSION",
    "PROTOCOL_METADATA_RECORD_VERSION",
    "PassiveCaptureSessionError",
    "BaselineWindowError",
    "BehaviorBaselineError",
    "DestinationLearningError",
    "PacketIngestionError",
    "PacketWindowError",
    "FlowEnrichmentError",
    "FlowReconstructionError",
    "FlowSessionTrackerError",
    "ProtocolMetadataError",
    "SENSITIVE_METADATA_KEYS",
    "SERVICE_ATTRIBUTION_RECORD_VERSION",
    "SERVICE_ATTRIBUTION_SAFETY_FLAGS",
    "SERVICE_BEHAVIOR_FINGERPRINT_RECORD_VERSION",
    "SERVICE_BEHAVIOR_FINGERPRINT_SAFETY_FLAGS",
    "SERVICE_FINGERPRINT_PROFILE_RECORD_VERSION",
    "SERVICE_FINGERPRINT_PROFILE_SAFETY_FLAGS",
    "SERVICE_PORTS",
    "SERVICE_PROTOCOL_HINTS",
    "SESSION_TRACKER_RECORD_VERSION",
    "SUPPORTED_TRANSPORTS",
    "TELEMETRY_SAFETY_FLAGS",
    "TelemetryInterfaceError",
    "TEMPORAL_ANOMALY_RECORD_VERSION",
    "TEMPORAL_ANOMALY_SAFETY_FLAGS",
    "AnomalyWindowError",
    "AdaptiveRiskError",
    "TemporalAnomalyError",
    "ServiceBehaviorFingerprintError",
    "ServiceFingerprintProfileError",
    "apply_adaptive_risk_weights",
    "build_anomaly_window_record",
    "build_anomaly_window_records",
    "build_adaptive_risk_api_response",
    "build_adaptive_risk_dashboard_record",
    "build_adaptive_risk_explanation",
    "build_adaptive_risk_export_record",
    "build_adaptive_risk_operator_panel",
    "build_adaptive_risk_record",
    "build_adaptive_risk_report",
    "build_adaptive_risk_rollup",
    "build_capture_plan_api_response",
    "build_baseline_entries",
    "build_baseline_observations",
    "build_baseline_window_record",
    "build_baseline_window_records",
    "build_behavior_baseline_api_response",
    "build_behavior_baseline_dashboard_record",
    "build_behavior_baseline_export_record",
    "build_behavior_baseline_operator_panel",
    "build_behavior_baseline_report",
    "build_behavioral_explanation_records",
    "build_behavioral_intelligence_api_response",
    "build_behavioral_intelligence_dashboard_record",
    "build_behavioral_intelligence_empty_state",
    "build_behavioral_intelligence_export_record",
    "build_behavioral_intelligence_operator_view",
    "build_behavioral_intelligence_status_panel",
    "build_behavioral_intelligence_summary",
    "build_behavioral_privacy_safety_summary",
    "build_behavioral_recommendation_records",
    "build_behavioral_state_summary",
    "build_anomaly_risk_context",
    "build_baseline_summary_rollup",
    "build_baseline_risk_context",
    "build_destination_risk_context",
    "build_destination_learning_records",
    "build_fingerprint_profile_records",
    "build_capture_plan_dashboard_record",
    "build_capture_targets",
    "build_dns_anomaly_hints",
    "build_dns_behavior_profiles",
    "build_dns_destination_api_response",
    "build_dns_destination_behavior_report",
    "build_dns_destination_dashboard_record",
    "build_dns_destination_export_record",
    "build_dns_destination_operator_panel",
    "build_dns_destination_behavior_rollup",
    "build_dns_query_record",
    "build_dns_response_record",
    "build_dns_timing_summary",
    "build_dns_visibility_api_response",
    "build_dns_visibility_dashboard_record",
    "build_dns_visibility_report",
    "build_enriched_flow_observation",
    "build_encrypted_dns_limitation_summary",
    "build_flow_api_response",
    "build_flow_counter_summary",
    "build_flow_enrichment_api_response",
    "build_flow_enrichment_dashboard_record",
    "build_flow_dashboard_record",
    "build_flow_quality_flags",
    "build_flow_session_records",
    "build_flow_state_transition",
    "build_interface_api_response",
    "build_interface_dashboard_record",
    "build_interface_resource_budget_summary",
    "build_cluster_federation_topology_summary",
    "build_bounded_update_interval_controls",
    "build_empty_live_telemetry_model",
    "build_federation_aware_telemetry_rollup",
    "build_flow_rate_summary",
    "build_interface_telemetry_summary",
    "build_live_topology",
    "build_live_topology_api_response",
    "build_live_topology_dashboard_record",
    "build_live_topology_rendering_summary",
    "build_live_telemetry_api_response",
    "build_live_telemetry_operator_summary",
    "build_packet_ingestion_api_response",
    "build_packet_ingestion_dashboard_record",
    "build_packet_ingestion_window",
    "build_passive_capture_session_plan",
    "build_process_inventory_api_response",
    "build_process_inventory_dashboard_record",
    "build_process_service_attribution_operator_panel",
    "build_process_service_attribution_report",
    "build_process_socket_inventory",
    "build_process_socket_inventory_from_platform",
    "build_metadata_governance_summary",
    "build_protocol_aware_topology_summary",
    "build_protocol_anomaly_summaries",
    "build_protocol_api_response",
    "build_protocol_dashboard_record",
    "build_protocol_fingerprint",
    "build_service_fingerprint_summary",
    "build_service_behavior_fingerprint_report",
    "build_service_fingerprint_api_response",
    "build_service_fingerprint_dashboard_record",
    "build_service_fingerprint_export_record",
    "build_service_fingerprint_operator_panel",
    "build_service_fingerprint_record",
    "build_service_fingerprint_records",
    "build_service_fingerprint_risk_context",
    "build_service_fingerprint_rollup",
    "build_packet_rate_summary",
    "build_protocol_distribution_summary",
    "build_temporal_anomaly_api_response",
    "build_temporal_anomaly_dashboard_record",
    "build_temporal_anomaly_export_record",
    "build_temporal_anomaly_operator_panel",
    "build_temporal_anomaly_report",
    "build_temporal_anomaly_rollup",
    "build_resource_usage_telemetry_summary",
    "build_rolling_flow_statistics",
    "build_sanitized_attribution_display",
    "build_service_attribution_api_response",
    "build_service_attribution_dashboard_record",
    "build_service_attribution_record",
    "build_stale_telemetry_state_model",
    "build_temporal_topology_summary",
    "build_telemetry_health_status_summary",
    "build_topology_health_summary",
    "build_topology_update_record",
    "build_risk_adjustment",
    "burst_anomaly_adjustment",
    "associate_flow_service",
    "attribute_process_to_flow",
    "classify_endpoint_scope",
    "classify_baseline_behavior",
    "classify_flow_endpoints",
    "classify_flow_persistence",
    "classify_flow_record",
    "classify_interface_capabilities",
    "classify_packet_address_family",
    "classify_packet_record",
    "classify_destination_ip_placeholder",
    "classify_dns_destination_behavior",
    "classify_service_fingerprint",
    "classify_service_fingerprint_profile",
    "classify_transport_protocol",
    "classify_window_packet",
    "classify_resolver",
    "clamp_risk_score",
    "correlate_domains_to_flows",
    "correlate_service_port_hint",
    "correlate_flow_topology_edges",
    "correlate_topology_drift",
    "deterministic_capture_plan_json",
    "deterministic_baseline_window_json",
    "deterministic_anomaly_window_json",
    "deterministic_behavior_baseline_json",
    "deterministic_behavior_operator_view_json",
    "deterministic_behavioral_intelligence_json",
    "deterministic_adaptive_risk_json",
    "deterministic_risk_weight_json",
    "deterministic_dns_correlation_json",
    "deterministic_destination_learning_json",
    "deterministic_dns_destination_behavior_json",
    "deterministic_dns_visibility_json",
    "deterministic_flow_enrichment_json",
    "deterministic_flow_json",
    "deterministic_flow_observation_json",
    "deterministic_interface_json",
    "deterministic_live_topology_json",
    "deterministic_live_topology_record_json",
    "deterministic_live_telemetry_json",
    "deterministic_packet_metadata_json",
    "deterministic_packet_window_json",
    "deterministic_process_attribution_json",
    "deterministic_protocol_fingerprint_json",
    "deterministic_protocol_metadata_json",
    "deterministic_service_attribution_json",
    "deterministic_service_behavior_fingerprint_json",
    "deterministic_service_fingerprint_profile_json",
    "deterministic_session_tracker_json",
    "deterministic_temporal_anomaly_json",
    "detect_burst_anomalies",
    "detect_rare_service_timing",
    "detect_volume_drift_hints",
    "detect_window_novelty",
    "enumerate_local_interfaces",
    "enrich_flow_records",
    "flow_digest",
    "flow_to_topology_edge",
    "infer_live_node_relationships",
    "infer_flow_direction",
    "infer_node_roles",
    "infer_protocol_hint",
    "low_confidence_dampening_adjustment",
    "mature_baseline_confidence_adjustment",
    "minimize_process_metadata",
    "new_behavior_adjustment",
    "normalize_address_family",
    "normalize_flow_key",
    "normalize_interface_address",
    "normalize_interface_metadata",
    "normalize_dns_answer",
    "normalize_packet_metadata",
    "normalize_packet_metadata_batch",
    "normalize_socket_record",
    "packet_metadata_digest",
    "pair_dns_queries_and_responses",
    "reconstruct_flow_record",
    "reconstruct_flows_from_packet_window",
    "safe_dns_text",
    "safe_destination_domain_summary",
    "safe_truncate_metadata_value",
    "sanitize_domain_name",
    "sanitize_metadata_fields",
    "stable_behavior_adjustment",
    "score_protocol_confidence",
    "score_anomaly_confidence",
    "score_baseline_confidence",
    "score_destination_learning_confidence",
    "score_flow_observation_confidence",
    "score_service_attribution_confidence",
    "score_service_fingerprint_confidence",
    "score_service_fingerprint_record",
    "summarize_capture_plan",
    "summarize_baseline_windows",
    "summarize_anomaly_windows",
    "summarize_adaptive_risk_records",
    "summarize_behavior_baselines",
    "summarize_enriched_flow_observations",
    "summarize_flow_sessions",
    "summarize_flows",
    "summarize_live_telemetry_panels",
    "summarize_interface_address_families",
    "summarize_interfaces",
    "summarize_packet_ingestion_window",
    "summarize_packet_metadata_records",
    "summarize_packet_sizes",
    "summarize_protocol_metadata",
    "summarize_dns_visibility_records",
    "summarize_dns_destination_behavior",
    "summarize_service_attributions",
    "summarize_service_fingerprint_profiles",
    "summarize_service_fingerprints",
    "summarize_socket_inventory",
    "summarize_temporal_anomalies",
    "track_flow_sessions",
    "unusual_destination_adjustment",
    "unusual_service_fingerprint_adjustment",
    "extract_dns_metadata",
    "extract_http_metadata",
    "extract_protocol_metadata",
    "extract_protocol_metadata_report",
    "extract_tls_metadata",
    "DNS_ALLOWED_FIELDS",
    "HTTP_ALLOWED_FIELDS",
    "TLS_ALLOWED_FIELDS",
    "validate_capture_session_plan_inputs",
    "validate_ip_address",
]
