...
- Milestone AA packaging and installers is planned in `docs/PHASE_159_164_PLAN.md`. Phases 159-160 are complete as baselines with Windows installer readiness records, PowerShell/MSI/ZIP/winget previews, Windows service previews, shortcut previews, macOS layout preview records, app bundle/pkg/dmg/Homebrew/CLI-only readiness, launchd previews, signing readiness, notarization readiness, uninstall previews, rollback previews, validation summaries, and export-safe safety fields. Remaining Phases 161-164 will add Linux package readiness, container deployment previews, secure updater readiness, and deployment wizard state models while preserving `portmap stack`, `portmap tui`, current dashboard behavior, no forced install actions, no service changes without operator approval, no admin escalation by default, no driver/kernel hooks, no credential storage, no private identifiers in docs or exports, and required rollback/uninstall previews.
