...
This roadmap summarizes the current direction after the Phase 170.5 baseline and the pre-Milestone W Milestone V live runtime bridge. `PORTMAP_AI_HANDOFF.md` remains the canonical implementation record, `docs/PHASE_HISTORY.md` records completed phase groups, `docs/MILESTONE_INTEGRATION.md` is the active integration guide, `docs/MILESTONE_R_INTEGRATION.md` summarizes Phase 105-110 behavioral intelligence integration, `docs/MILESTONE_S_INTEGRATION.md` summarizes Phase 111-116 historical persistence integration, `docs/MILESTONE_T_INTEGRATION.md` summarizes Phase 117-122 operationalization and deployment integration, `docs/MILESTONE_U_INTEGRATION.md` summarizes Phase 123-128 security foundation integration, `docs/MILESTONE_V_INTEGRATION.md` summarizes Phase 129-134 deep network flow intelligence integration, `docs/MILESTONE_W_INTEGRATION.md` summarizes Phase 135-140 autonomous response and policy engine integration, `docs/MILESTONE_X_INTEGRATION.md` summarizes Phase 141-146 visual intelligence integration, `docs/MILESTONE_Y_INTEGRATION.md` summarizes Phase 147-152 threat intelligence and detection expansion integration, `docs/MILESTONE_Z_INTEGRATION.md` summarizes Phase 153-158 scalability and distributed infrastructure integration, `docs/PHASE_135_140_PLAN.md` defines the completed Milestone W autonomous response and policy engine baseline, `docs/PHASE_141_146_PLAN.md` defines the completed Milestone X visual intelligence baseline, `docs/PHASE_147_152_PLAN.md` defines the completed Milestone Y threat intelligence and detection expansion baseline, `docs/PHASE_153_158_PLAN.md` defines the completed Milestone Z scalability and distributed infrastructure baseline, `docs/PHASE_159_164_PLAN.md` defines the completed Milestone AA packaging and installers baseline, `docs/PHASE_165_170_PLAN.md` defines the completed Milestone AB compliance and governance baseline, `docs/tui_dashboard.md` documents the Phase 170.5 multi-tab TUI navigation bridge, `docs/milestone_v_live_runtime_integration.md` documents the runtime bridge from live socket snapshots into Milestone V operator views, `docs/COMPLETION_ROADMAP.md` defines the remaining end-to-end completion path, and `docs/PORTMAP_AI_FINAL_ROADMAP.md` captures the longer production-launch roadmap from Milestone U onward.
...
| 148 | DNS threat analytics: metadata-only domain pattern records, domain hashing, redacted previews, resolver behavior summaries, local IOC match integration, DNS analytics rollups, source-mode preservation, and no DNS lookups, external feeds, raw DNS history, blocking, malicious flags, final threat verdicts, or enforcement | Complete baseline |
| 149 | Threat signature framework: local metadata-only signature records, validation, unsafe condition rejection, deterministic matching across IOC, DNS, flow, protocol, attribution, topology, runtime, and composite metadata, and no external feeds, malicious flags, final threat verdicts, blocking, or enforcement | Complete baseline |
| 150 | AI correlation layer: metadata-only evidence chains, deterministic local correlation summaries, IOC/DNS/signature, flow/attribution/drift, topology/policy/risk, remediation/guardrail, and composite chain rollups, explanation points, and no external AI calls, final threat verdicts, blocking, or enforcement | Complete baseline |
| 151 | Threat scoring expansion: advisory scoring weights, bounded advisory score records, IOC/DNS/signature/correlation/flow/attribution/drift/topology/runtime/remediation/guardrail breakdowns, confidence aggregation, explanation points, and no malicious labels, final threat verdicts, blocking, or enforcement | Complete baseline |
| 152 | Threat hunting query engine: local metadata-only query records, equality/contains/confidence/severity/source-mode filters, bounded result limits, deterministic matched-record summaries, source-scope rollups, and no external queries, malicious labels, final threat verdicts, blocking, or enforcement | Complete baseline |

## Current Implementation State

Phases 0-152 are implemented locally in the working tree and documented as complete baselines. A complete baseline means the foundational implementation is operational and tested, while future work may expand integration depth, production hardening, installer packaging, and operator workflows.
...
- Safety guardrails now model approval gates, rollback gates, blast-radius gates, provider readiness gates, confidence gates, runtime health gates, policy scope gates, emergency-stop gates, and rollback simulations as preview-only records with no rollback, backup, file, firewall, service, process, or enforcement side effects.
- Autonomous enforcement mode modeling now defines monitor, supervised, autonomous-preview, and hardened-preview mode records plus autonomy controls, explicit approval/audit/emergency-stop requirements, safer-mode recommendations, containment disabled, and enforcement inactive.
...
- Milestone Z scalability and distributed infrastructure is complete as a baseline through Phase 158, summarized in `docs/MILESTONE_Z_INTEGRATION.md`, and tracked in `docs/PHASE_153_158_PLAN.md`. Phases 153-158 provide metadata-only local bus envelopes, telemetry topics, bounded in-memory queue summaries, dropped-by-bound previews, retry/backoff metadata, fanout readiness, retention tier records, storage readiness summaries, utilization and pressure states, telemetry bus queue input summaries, write/read capacity previews, compaction previews, worker group records, cluster size previews, shard and partition planning previews, worker distribution summaries, capacity forecasts, resource budget records, CPU/memory/storage/telemetry/worker utilization summaries, adaptive sampling previews, load-shedding previews, deployment budget guidance, edge profile records, Raspberry Pi and Linux ARM readiness, offline/degraded operation summaries, gateway and branch collector previews, relay session records, routing previews, tenant isolation previews, relay capacity planning, and export-safe scaling, optimization, edge, and cloud relay readiness records while avoiding external broker dependencies, live database dependencies, runtime data writes, deletion, destructive compaction, live cloud provisioning, cluster creation, cloud APIs, telemetry routing changes, telemetry throttling, sampling changes, worker-count changes, worker deployment, collection changes, relay creation, network connections, telemetry forwarding, enforcement, firewall/process/service changes, credential storage, and private identifier export.
- Milestone AA packaging and installers is complete as a baseline through Phase 164 and planned in `docs/PHASE_159_164_PLAN.md`. Phases 159-164 provide Windows installer readiness records, PowerShell/MSI/ZIP/winget previews, Windows service previews, shortcut previews, macOS layout preview records, app bundle/pkg/dmg/Homebrew/CLI-only readiness, launchd previews, signing readiness, notarization readiness, Linux layout preview records, DEB/RPM/tarball/APT repository/CLI-only readiness, systemd previews, Raspberry Pi readiness, Linux ARM readiness, container profile previews, Docker/Compose/Podman/containerd-preview readiness, volume/network/environment previews, resource limit recommendations, update channel records, checksum/signature/version readiness, staged rollout previews, deployment wizard state models, guided setup summaries, environment/profile/install-method recommendations, TUI screen recommendations, uninstall previews, rollback previews, validation summaries, and export-safe safety fields while preserving `portmap stack`, `portmap tui`, current dashboard behavior, no forced install actions, no service changes without operator approval, no admin escalation by default, no driver/kernel hooks, no credential storage, no private identifiers in docs or exports, and required rollback/uninstall previews.
- Milestone AB compliance and governance is complete as a baseline through Phase 170 and tracked in `docs/PHASE_165_170_PLAN.md`. Phases 165-170 provide metadata-only audit event records, daily log rotation readiness, Last Export Summary records, export validation summaries, expected/observed/missing file summaries, schema/sensitive-data/artifact check states, retention previews, compression previews, deletion previews, compliance profile records, evidence expectation records, audit/export/retention/privacy expectation summaries, operator responsibility summaries, fixed false certification flags, data classification records, privacy boundary summaries, retention control summaries, redaction readiness, export governance summaries, operator action records, approval summaries, reviewer chain summaries, role mapping summaries, accountability evidence summaries, security review records, checklist summaries, runtime/deployment/packaging review summaries, governance/accountability/compliance review integration, privacy review records, redaction/export privacy summaries, consent and notice readiness, legal safeguard notes, privacy recommendations, and export-safe safety fields. The completed baseline preserves export-safe, privacy-aware, operator-controlled behavior with no destructive deletion, credential storage, legal advice, legal certification claims, enforcement, authorization decisions, security scanning, vulnerability detection, role assignment, identity storage, firewall/process/service changes, private identifier export, private export reads by default, filesystem reads or writes, or current runtime/TUI behavior changes. Runtime Export Validation Panel remains deferred until tabbed or multi-screen TUI navigation exists.
- Phase 170.5 adds the multi-tab TUI navigation bridge documented in `docs/tui_dashboard.md`. Dashboard remains the default live runtime tab with compact risk overview only, and Phase 170.5A.X+2 makes Risk a one-screen read-only SOC analyst workspace with Dashboard-style section headers, bounded `DataTable` regions, a merged Risk Status table, dominant Active Risk Findings table with existing-data Severity/Asset/Service/Finding/Score/Action/Time columns, bottom Signals/Feed/Timeline tables, and one-line allowlist/safety footer status. Exports, Governance, Deployment, AI, and Packet tabs provide readiness placeholders for future operator-visible surfaces without packet capture, new collectors, network behavior, governance enforcement, installer execution, extra export writes, remediation execution, blocking, or runtime stack behavior changes.
- Milestones AC through AD remain reserved for AI intelligence evolution and commercial launch readiness as tracked in `docs/PORTMAP_AI_FINAL_ROADMAP.md`.
