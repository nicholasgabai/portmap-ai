...
- Phase 170.5 adds the multi-tab TUI navigation bridge documented in `docs/tui_dashboard.md`. Dashboard remains the default live runtime tab with compact risk overview only, and Phase 170.5A.X makes Risk a one-screen read-only SOC analyst workspace with a merged Risk Status strip, dominant Active Risk Findings table, existing-data Severity/Asset/Service/Finding/Score/Action columns, bottom Signals/Feed/Timeline columns, and one-line allowlist/safety footer status. Exports, Governance, Deployment, AI, and Packet tabs provide readiness placeholders for future operator-visible surfaces without packet capture, new collectors, network behavior, governance enforcement, installer execution, extra export writes, remediation execution, blocking, or runtime stack behavior changes.
