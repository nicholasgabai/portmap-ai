...
- Milestone Z scalability and distributed infrastructure is planned in `docs/PHASE_153_158_PLAN.md`. Phases 153-157 are complete as baselines with metadata-only local bus envelopes, telemetry topics, bounded in-memory queue summaries, dropped-by-bound previews, retry/backoff metadata, fanout readiness, retention tier records, storage readiness summaries, utilization and pressure states, telemetry bus queue input summaries, write/read capacity previews, compaction previews, worker group records, cluster size previews, shard and partition planning previews, worker distribution summaries, capacity forecasts, resource budget records, CPU/memory/storage/telemetry/worker utilization summaries, adaptive sampling previews, load-shedding previews, deployment budget guidance, edge profile records, Raspberry Pi and Linux ARM readiness, offline/degraded operation summaries, gateway and branch collector previews, and export-safe scaling, optimization, and edge readiness records. Phase 158 will continue with cloud relay readiness while avoiding external broker dependencies, live database dependencies, runtime data writes, deletion, destructive compaction, live cloud provisioning, cluster creation, cloud APIs, telemetry routing changes, telemetry throttling, sampling changes, worker-count changes, worker deployment, collection changes, relay creation, enforcement, firewall/process/service changes, credential storage, and private identifier export.
