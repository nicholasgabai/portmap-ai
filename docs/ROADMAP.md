...
- Milestone Z scalability and distributed infrastructure is planned in `docs/PHASE_153_158_PLAN.md`. Phases 153-156 are complete as baselines with metadata-only local bus envelopes, telemetry topics, bounded in-memory queue summaries, dropped-by-bound previews, retry/backoff metadata, fanout readiness, retention tier records, storage readiness summaries, utilization and pressure states, telemetry bus queue input summaries, write/read capacity previews, compaction previews, worker group records, cluster size previews, shard and partition planning previews, worker distribution summaries, capacity forecasts, resource budget records, CPU/memory/storage/telemetry/worker utilization summaries, adaptive sampling previews, load-shedding previews, deployment budget guidance, and export-safe scaling and optimization readiness records. Phases 157-158 will continue with edge worker modes and cloud relay readiness while avoiding external broker dependencies, live database dependencies, runtime data writes, deletion, destructive compaction, live cloud provisioning, cluster creation, cloud APIs, telemetry routing changes, telemetry throttling, sampling changes, worker-count changes, enforcement, firewall/process/service changes, credential storage, and private identifier export.
