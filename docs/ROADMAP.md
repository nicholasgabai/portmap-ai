...
- Milestone Z scalability and distributed infrastructure is planned in `docs/PHASE_153_158_PLAN.md`. Phases 153-154 are complete as baselines with metadata-only local bus envelopes, telemetry topics, bounded in-memory queue summaries, dropped-by-bound previews, retry/backoff metadata, fanout readiness, retention tier records, storage readiness summaries, utilization and pressure states, telemetry bus queue input summaries, write/read capacity previews, compaction previews, and export-safe records. Phases 155-158 will continue with horizontal scaling, resource optimization, edge worker modes, and cloud relay readiness while avoiding external broker dependencies, live database dependencies, runtime data writes, deletion, destructive compaction, live cloud provisioning, enforcement, firewall/process/service changes, credential storage, and private identifier export.
