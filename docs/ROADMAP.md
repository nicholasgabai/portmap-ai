...
- Milestone Z scalability and distributed infrastructure is planned in `docs/PHASE_153_158_PLAN.md`. Phase 153 is complete as a baseline with metadata-only local bus envelopes, telemetry topics, bounded in-memory queue summaries, dropped-by-bound previews, retry/backoff metadata, fanout readiness, and export-safe bus summaries. Phases 154-158 will continue with high-volume storage readiness, horizontal scaling, resource optimization, edge worker modes, and cloud relay readiness while avoiding external broker dependencies, live cloud provisioning, destructive storage actions, enforcement, firewall/process/service changes, credential storage, and private identifier export.
