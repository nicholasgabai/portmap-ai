...
- Phase 170.5 adds the multi-tab TUI navigation bridge documented in `docs/tui_dashboard.md`. Dashboard remains the default live runtime tab with compact risk overview only, Phase 170.5A.5 makes Risk a one-screen read-only investigation dashboard centered on capped Active Risk Findings and styled with Dashboard-style dense section headers from existing runtime data, and Exports, Governance, Deployment, AI, and Packet tabs provide readiness placeholders for future operator-visible surfaces without packet capture, new collectors, network behavior, governance enforcement, installer execution, extra export writes, remediation execution, blocking, or runtime stack behavior changes.
