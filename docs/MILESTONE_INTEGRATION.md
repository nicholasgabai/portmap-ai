...
Phase 170.5A.5 makes the Risk tab a one-screen dashboard instead of a scroll-heavy report. The live Risk view removes the vertical scroll dependency, keeps dense Dashboard-style section headers, caps Active Risk Findings at 6 rows, Recent Remediation Feed at 5 rows, Top Risk Signals at 5 rows, Risk Timeline at 3 rows, and combines allowlist plus safety status into a short footer note. Dashboard remains unchanged as the shortcut `1` default overview, Risk remains shortcut `2`, and the refresh loop, footer actions, and read-only safety boundary are preserved with no collectors, packet capture, file writes, enforcement, blocking, remediation execution, network behavior, private-data persistence, runtime action changes, or firewall/process/service changes.
