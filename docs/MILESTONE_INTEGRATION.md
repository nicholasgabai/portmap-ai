...
Milestone Y planning is tracked in `docs/PHASE_147_152_PLAN.md`. Phases 147-150 now extend visual intelligence into metadata-only threat intelligence and detection readiness with IOC models, DNS threat analytics, local signature matching, and deterministic AI correlation evidence chains. Remaining Phase 151-152 work covers advisory threat scoring expansion and local threat-hunting query models while remaining free of final threat verdicts, blocking, enforcement, external threat-feed lookups, external AI calls, packet payload storage, raw DNS history, credential storage, and private identifier export.
