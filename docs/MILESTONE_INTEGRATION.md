...
| Threat Intelligence and Detection Expansion | 147-149 | Metadata-only IOC records, bounded inventories, local matching, hash-only exports, DNS domain pattern records, resolver behavior summaries, IOC match integration, DNS analytics state rollups, local signature records, deterministic signature matching, composite signal support, source-mode preservation, export-safe dictionaries, and no external lookups, remote feeds, malicious flags, final threat verdicts, blocking, enforcement, raw DNS history, packet payload storage, or private identifier export | Complete baseline in progress |
...
Milestone Y planning is tracked in `docs/PHASE_147_152_PLAN.md`. Phases 147-149 now extend visual intelligence into metadata-only threat intelligence and detection readiness with IOC models, DNS threat analytics, and local signature matching. Remaining Phase 150-152 work covers AI correlation records, advisory threat scoring expansion, and local threat-hunting query models while remaining free of final threat verdicts, blocking, enforcement, external threat-feed lookups, packet payload storage, raw DNS history, credential storage, and private identifier export.
