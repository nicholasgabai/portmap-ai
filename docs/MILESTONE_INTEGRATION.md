...
| Deep Network Flow Intelligence | 129-131 | Bidirectional flow reconstruction, normalized session tracking, inbound/outbound/local-loopback direction inference, source-mode preservation, flow pairs, flow relationships, packet/socket/session/DNS/protocol/process/service/topology correlation records, cross-node relationship graphs, recurring peer interaction scoring, topology distance, advisory lateral analysis, live Unknown/Unattributed fallbacks, fixture/simulated-only demo labels, reconstruction confidence, relationship strength, and no payload inspection, raw packet storage, PCAP generation, graph database dependency, threat verdict engine, DPI, credential storage, raw DNS browsing-history logging, or automatic enforcement | Complete baseline |
