...
Milestone Y planning is tracked in `docs/PHASE_147_152_PLAN.md`. It should extend visual intelligence into metadata-only threat intelligence and detection readiness with IOC models, DNS threat analytics, local signatures, AI correlation records, advisory threat scoring expansion, and local threat-hunting query models while remaining free of final threat verdicts, blocking, enforcement, external threat-feed lookups, packet payload storage, raw DNS history, credential storage, and private identifier export.
