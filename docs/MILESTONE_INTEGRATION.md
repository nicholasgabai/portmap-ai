...
Milestone AB planning is tracked in `docs/PHASE_165_170_PLAN.md`. Phase 165 now provides readiness-first audit event records, daily log rotation readiness, Last Export Summary records, export validation summaries, expected/observed/missing file summaries, schema/sensitive-data/artifact check state summaries, retention previews, compression previews, deletion previews, and export-safe audit views while preserving metadata-only, export-safe, privacy-aware, operator-controlled behavior. Runtime Export Validation Panel is deferred until tabbed or multi-screen TUI navigation exists. Remaining Phases 166-170 will add compliance profiles, data governance controls, operator accountability records, security review summaries, privacy-safe export summaries, consent/operator notice records, and legal safeguard notes. The plan avoids destructive deletion, credential storage, legal advice or certification claims, enforcement, firewall/process/service changes, private identifier export, and current runtime/TUI behavior changes.
