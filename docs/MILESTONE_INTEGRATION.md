...
| Deep Network Flow Intelligence | 129 | Bidirectional flow reconstruction, normalized session tracking, inbound/outbound/local-loopback direction inference, source-mode preservation, flow pairs, flow relationships, transient and recurring session summaries, reconstruction confidence, relationship strength, and no payload inspection, PCAP generation, DPI, credential storage, or automatic enforcement | Complete baseline |
