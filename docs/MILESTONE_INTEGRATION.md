...
Phase 170.5A.6 restores useful Risk data density inside the one-screen layout. Risk keeps separate Risk Summary, Queue Summary, Active Risk Findings, Top Risk Signals, Recent Remediation Feed, Risk Timeline, Allowlist Status, and Safety Boundary sections with compact table rows. Active Risk Findings is capped at 8 rows and uses the largest center area, while Top Risk Signals, Recent Remediation Feed, and Risk Timeline share the bottom row as left/center/right columns. Allowlist Status plus Safety Boundary remain capped at 2 lines each. The refresh loop, footer actions, and read-only safety boundary are preserved with no collectors, packet capture, file writes, enforcement, blocking, remediation execution, network behavior, private-data persistence, runtime action changes, or firewall/process/service changes.
