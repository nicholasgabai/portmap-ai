...
Milestone AA planning is tracked in `docs/PHASE_159_164_PLAN.md`. Phases 159-161 now provide readiness-first Windows installer records, PowerShell/MSI/ZIP/winget install plan previews, Windows service previews, Start Menu/Desktop shortcut previews, macOS layout preview records, app bundle/pkg/dmg/Homebrew/CLI-only package readiness, launchd previews, signing readiness summaries, notarization readiness summaries, Linux layout preview records, DEB/RPM/tarball/APT repository/CLI-only package readiness, systemd previews, Raspberry Pi readiness summaries, Linux ARM readiness summaries, uninstall previews, rollback previews, validation summaries, and export-safe command/path previews while preserving the validated `portmap stack`, `portmap tui`, and dashboard baseline. Remaining Phases 162-164 will define container deployment, secure updater, and deployment wizard records. The plan requires rollback and uninstall previews and keeps installer work non-invasive: no forced install actions, no service changes without operator approval, no admin escalation by default, no driver or kernel hooks, no credential storage, no private identifier export, and no destructive install behavior.
