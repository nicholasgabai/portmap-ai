...
| Milestone V Live Runtime Bridge | `core_engine.runtime.milestone_v_bridge`, `core_engine.dispatcher`, `gui.visualization` | Convert bounded current worker socket snapshots into reconstructed sessions, flow rows, metadata/process correlations, relationship edges, attribution candidates, drift records, topology records, runtime counters, and TUI Traffic Flows/Topology Edges while preserving socket-only limits, no payload inspection, no PCAP generation, and no live dummy labels. |
