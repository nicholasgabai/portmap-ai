...
Milestone AB planning is tracked in `docs/PHASE_165_170_PLAN.md`. Phases 165-166 now provide readiness-first audit event records, daily log rotation readiness, Last Export Summary records, export validation summaries, expected/observed/missing file summaries, schema/sensitive-data/artifact check state summaries, retention previews, compression previews, deletion previews, export-safe audit views, compliance profile records, evidence expectation records, audit/export/retention/privacy expectation summaries, operator responsibility summaries, and fixed false certification flags while preserving metadata-only, export-safe, privacy-aware, operator-controlled behavior. Runtime Export Validation Panel is deferred until tabbed or multi-screen TUI navigation exists. Remaining Phases 167-170 will add data governance controls, operator accountability records, security review summaries, privacy-safe export summaries, consent/operator notice records, and legal safeguard notes. The plan avoids destructive deletion, credential storage, legal advice or certification claims, enforcement, firewall/process/service changes, private identifier export, and current runtime/TUI behavior changes.
