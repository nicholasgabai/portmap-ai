...
Milestone AB planning is tracked in `docs/PHASE_165_170_PLAN.md`. Phases 165-170 will add readiness-first audit logging, daily rotating log plans, Last Export Summary records, compliance profiles, data governance controls, operator accountability records, security review summaries, privacy-safe export summaries, consent/operator notice records, and legal safeguard notes while preserving metadata-only, export-safe, privacy-aware, operator-controlled behavior. Runtime Export Validation Panel is deferred until tabbed or multi-screen TUI navigation exists. The plan avoids destructive deletion, credential storage, legal advice or certification claims, enforcement, firewall/process/service changes, private identifier export, and current runtime/TUI behavior changes.
