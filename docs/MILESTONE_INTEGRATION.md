...
| Deep Network Flow Intelligence | 129-130 | Bidirectional flow reconstruction, normalized session tracking, inbound/outbound/local-loopback direction inference, source-mode preservation, flow pairs, flow relationships, packet/socket/session/DNS/protocol/process/service/topology correlation records, live Unknown/Unattributed fallbacks, fixture/simulated-only demo labels, reconstruction confidence, relationship strength, and no payload inspection, raw packet storage, PCAP generation, DPI, credential storage, raw DNS browsing-history logging, or automatic enforcement | Complete baseline |
