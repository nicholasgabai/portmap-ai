...
Phase 170.5 adds multi-tab TUI navigation as a bridge from Milestone AB into Milestone AC and future Milestone AE. Dashboard remains the default current-runtime tab, while Risk, Exports, Governance, Deployment, AI, and Packet tabs provide readiness placeholders with keyboard shortcuts 2-7. The Packet tab is placeholder-only until Milestone AE, and the Export Validation Panel remains future work under the Exports tab. This navigation layer does not add packet capture, collectors, network behavior, governance enforcement, installer/deployment execution, extra export writes, or runtime behavior changes.
