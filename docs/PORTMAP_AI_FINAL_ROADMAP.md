...
- Passive interface discovery, bounded packet metadata windows, flow reconstruction, protocol metadata extraction, dynamic topology correlation, real-time telemetry views, source-mode labeling, current-snapshot scan deduplication, and the Milestone V live runtime bridge from worker socket snapshots into TUI-visible flow/topology summaries.
