...
Phase 170.5 is complete as a TUI navigation bridge and documented in `docs/tui_dashboard.md`. It keeps the current dashboard as the default tab and adds Risk, Exports, Governance, Deployment, AI, and Packet readiness placeholders with stable keyboard shortcuts. The Exports tab reserves space for the future Runtime Export Validation Panel, and the Packet tab remains placeholder-only until future Milestone AE. This phase adds no packet capture, collectors, network behavior, governance enforcement, installer execution, extra export writes, or runtime stack behavior changes.
